...
# The ratis-hermes container bind-mounts the host's ~/.hermes at this path.
# We drop our prompt files under <host>/.hermes/state/postmortem-prompts/ and
# reference them from inside the container as /opt/data/state/postmortem-prompts/
# — this avoids the docker-exec argv limit (~128KB) for large transcripts.
DEFAULT_HOST_PROMPT_DIR = Path(
    os.environ.get(
        "HERMES_POSTMORTEM_HOST_PROMPT_DIR",
        str(Path.home() / ".hermes" / "state" / "postmortem-prompts"),
    )
)
...
class CodexClient:
    ...
    def is_available(self) -> bool:
        """``docker`` must be on PATH; we don't probe the container here.

        The container check happens lazily on first ``call`` — if the container
        is down we surface the stderr to the audit log and return an error.
        """
