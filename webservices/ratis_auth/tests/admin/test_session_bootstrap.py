"""TDD coverage for POST /api/v1/admin/session-bootstrap (AU service, Module 10 PR 5).

Tests:
- Correct ADMIN_API_KEY → 200 + {ott, redirect_url}
- Wrong key → 403 (matches verify_admin_key convention in existing codebase)
- No key → 403
...
@pytest.fixture
def mock_redis():
    """Fake Redis that always succeeds for SET NX (OTT not yet used)."""
    redis = MagicMock()
...
class TestSessionBootstrapOk:
    ...
    def test_ott_is_valid_hs256_jwt(self, client_with_mock_redis) -> None:
        ...
        assert payload["redirect"] == "/admin/users"
        uuid.UUID(payload["jti"])  # must be valid UUID

    def test_redirect_url_contains_ott(self, client_with_mock_redis) -> None:
        """The redirect_url contains the OTT query param."""
        resp = client_with_mock_redis.post(
            "/api/v1/admin/session-bootstrap",
            json={"redirect": "/admin/users"},
        )
        assert resp.status_code == 200, resp.text
        body = resp.json()
        assert "ott=" in body["redirect_url"]
        assert body["ott"] in body["redirect_url"]

    def test_default_redirect(self, client_with_mock_redis) -> None:
        """POST without redirect body uses a default path."""
        resp = client_with_mock_redis.post(
            "/api/v1/admin/session-bootstrap",
            json={},
        )
