# batch/ratis_batch_reconciliation/tests/conftest.py
import os
import uuid
...
os.environ["REWARDS_BASE_URL"] = "http://rewards.test"  # required by deferred_gift_cards
os.environ["SENTRY_DSN"] = ""  # DSN vide = Sentry silent en tests

import pytest
import ratis_core.models  # noqa: F401 — registers all ORM models
...
def engine():
    ...
    with eng.connect() as conn:
        try:
            ...
            conn.execute(text("SELECT 1 FROM user_cashback_balance LIMIT 0"))
            ...
            raise RuntimeError(
                ...
                "produce expected tables (users / user_cab_balance / "
                "user_cashback_balance / cabecoin_transactions / "
                "cashback_transactions). Likely a model module is not imported."
            ) from exc
    yield eng
    eng.dispose()
...
def make_user(session_factory):
    """Insert a minimal user row with cab + cashback balance rows, return its UUID."""
    ...
    def _make():
        ...
        with session_factory() as db:
            ...
            db.execute(
                text("""
                INSERT INTO user_cab_balance (user_id, balance)
                VALUES (:uid, 0)
            """),
                {"uid": uid},
            )
            db.execute(
                text("""
                INSERT INTO user_cashback_balance (user_id, balance)
                VALUES (:uid, 0)
            """),
                {"uid": uid},
            )
