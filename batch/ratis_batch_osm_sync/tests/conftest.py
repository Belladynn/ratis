...
# Add batch root to sys.path so `import osm_sync` works
...
def engine():
    ...
    with eng.connect() as conn:
        ...
        # cube + earthdistance are required by batch_shared.store_consolidation.find_match
        # (ll_to_earth / earth_distance / earth_box functions — installed in prod
        # via migration 20260511_0900_pg_earthdistance).
    ...
    with eng.connect() as conn:
        try:
            ...
            raise RuntimeError(
                ...
                "produce expected tables (stores / batch_sync_log). "
    ...
    #
    # Note (2026-04-27) : `uq_stores_phone` is intentionally NOT recreated —
    # phone is no longer unique per the retailer-signal refactor (multiple
    # franchise stores share the same corporate standard). Tests rely on
    # this absence to insert duplicate phones. See
